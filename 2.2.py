...
class Game:
    def __init__(self, game_id, data):
        self.gameID = int(game_id.split(' ')[1])
        self.list_sets = self.parse_data(data)
        self.setCountGreen, self.setCountRed, self.setCountBlue = self.calculate_minimum_cubes()
        self.gamePower = self.setCountGreen * self.setCountRed * self.setCountBlue
    ...
    def parse_data(data):
        ...
        for s in sets:
            ...
            parsed_sets.append((green, red, blue))
        return parsed_sets

    def calculate_minimum_cubes(self):
        # Calculate the maximum count of each color in any set
        max_green = max([s[0] for s in self.list_sets])
        max_red = max([s[1] for s in self.list_sets])
        max_blue = max([s[2] for s in self.list_sets])
        return max_green, max_red, max_blue

    def isPossible(self):
        max_red, max_green, max_blue = 12, 13, 14
        for green, red, blue in self.list_sets:
            if red > max_red or green > max_green or blue > max_blue:
                return False
        return True


# Process the data and compute the minimum number of cubes and game power for each game
games_data = data.strip().split('\n')
game_details = {}
for game_info in games_data:
    game_id, game_data = game_info.split(': ')
    game = Game(game_id, game_data)
    game_details[game.gameID] = {
        "Minimum Green": game.setCountGreen,
        "Minimum Red": game.setCountRed,
        "Minimum Blue": game.setCountBlue,
        "Game Power": game.gamePower
    }


def calculate_total_game_power(games_data):
    """
    Calculate the total game power for a list of game data.

    :param games_data: A string containing game data.
    :return: The total game power for all games.
    """
    ...
    for game_info in games:
        game_id, game_data = game_info.split(': ')
        game = Game(game_id, game_data)
        total_game_power += game.gamePower
    return total_game_power

# Testing the function with the provided test data
total_game_power = calculate_total_game_power(data)
