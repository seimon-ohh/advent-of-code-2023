...
class Game:
    def __init__(self, game_id, data):
        self.gameID = int(game_id.split(' ')[1])  # Extract only the number from the game ID
        self.list_sets = self.parse_data(data)
        self.setCountGreen = sum([s[0] for s in self.list_sets])
        self.setCountRed = sum([s[1] for s in self.list_sets])
        self.setCountBlue = sum([s[2] for s in self.list_sets])

    @staticmethod
    def parse_data(data):
        # Convert data string into a list of tuples
        sets = data.split('; ')
        parsed_sets = []
        for s in sets:
...
for game_info in games_data:
    ...
    game_results[game.gameID] = game.isPossible()

# Sum of IDs of games that are possible
sum_possible_game_ids = sum(game_id for game_id, is_possible in game_results.items() if is_possible)
